...
//  clock_t start_time = clock();//计时
  ...
  ROS_INFO("Raw map has been updated with a new point cloud in %f s.", duration.toSec());//原来是ROS_DEBUG

  scopedLockForRawData.unlock();

//  clock_t finish_time = clock();
//  double total_time = (double)(finish_time - start_time) / CLOCKS_PER_SEC;
//  std::cout << "rawmap cost time" << total_time * 1000 << "ms" << std::endl;
  return true;
}
...
//  clock_t finish_time = clock();//结束计时
//  double total_time = (double)(finish_time - start_time) / CLOCKS_PER_SEC;
//  std::cout << "fuse cost time" << total_time * 1000 << "ms" << std::endl;

  // Copy raw elevation map data for safe multi-threading. 15848.4ms
  boost::recursive_mutex::scoped_lock scopedLockForRawData(rawMapMutex_);
  auto rawMapCopy = rawMap_;//5.594ms
  scopedLockForRawData.unlock();
  const ros::WallDuration duration(ros::WallTime::now() - methodStartTime);

  // More initializations. 0.012ms
  ...
  ROS_INFO("Elevation map has been fused in %f s.", duration.toSec());//原来是ROS_DEBUG
...
void ElevationMap::visibilityCleanup(const ros::Time& updatedTime)
{
  ...
  clock_t finish_time = clock();
  double total_time = (double)(finish_time - start_time) / CLOCKS_PER_SEC;
  std::cout << "clean cost time" << total_time * 1000 << "ms" << std::endl;
...
bool ElevationMap::publishFusedElevationMap(double threshold)
{
  ...
  fusedMapCopy.add("height_color");
//  fusedMapCopy.add("max_height");
//  fusedMapCopy.add("min_height");
  for (GridMapIterator iterator(fusedMapCopy); !iterator.isPastEnd(); ++iterator){
    if (!fusedMapCopy.isValid(*iterator)) continue;
    const auto& elevation_height = fusedMapCopy.at("uncertainty_range", *iterator);
    auto& colorHeight = fusedMapCopy.at("height_color", *iterator);
    if(elevation_height > threshold)
      colorHeight = 1.0;//red
    else
      colorHeight = 0.1;//green65280/3289650
  }
//  for (GridMapIterator iterator(fusedMapCopy); !iterator.isPastEnd(); ++iterator) {
//    if (!fusedMapCopy.isValid(*iterator)) continue;
//    const auto& lowestScanPoint = fusedMapCopy.at("lowest_scan_point", *iterator);
//    const auto& sensorXatLowestScan = fusedMapCopy.at("sensor_x_at_lowest_scan", *iterator);
//    const auto& sensorYatLowestScan = fusedMapCopy.at("sensor_y_at_lowest_scan", *iterator);
//    const auto& sensorZatLowestScan = fusedMapCopy.at("sensor_z_at_lowest_scan", *iterator);
//    if (std::isnan(lowestScanPoint)) continue;
//    Index indexAtSensor;
//    if(!fusedMapCopy.getIndex(Position(sensorXatLowestScan, sensorYatLowestScan), indexAtSensor)) continue;
//    Position point;
//    fusedMapCopy.getPosition(*iterator, point);
//    float pointDiffX = point.x() - sensorXatLowestScan;
//    float pointDiffY = point.y() - sensorYatLowestScan;
//    float distanceToPoint = sqrt(pointDiffX * pointDiffX + pointDiffY * pointDiffY);
//    if (distanceToPoint > 0.0) {
//      for (grid_map::LineIterator iterator(fusedMapCopy, indexAtSensor, *iterator); !iterator.isPastEnd(); ++iterator) {
//        Position cellPosition;
//        fusedMapCopy.getPosition(*iterator, cellPosition);
//        const float cellDiffX = cellPosition.x() - sensorXatLowestScan;
//        const float cellDiffY = cellPosition.y() - sensorYatLowestScan;
//        const float distanceToCell = distanceToPoint - sqrt(cellDiffX * cellDiffX + cellDiffY * cellDiffY);
//        const float maxHeightPoint = lowestScanPoint + (sensorZatLowestScan - lowestScanPoint) / distanceToPoint * distanceToCell;
//        auto& cellMaxHeight = fusedMapCopy.at("max_height", *iterator);
//        auto& cellMinHeight = fusedMapCopy.at("min_height", *iterator);
//        if (std::isnan(cellMaxHeight) || cellMaxHeight > maxHeightPoint) {
//          cellMaxHeight = maxHeightPoint;
//        }
//        if(cellMaxHeight < maxHeightPoint)
//        {
//          cellMinHeight = ;
//        }
//      }
